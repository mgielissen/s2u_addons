...
class CustomerPortal(CustomerPortal):
    ...
    def _prepare_portal_layout_values(self):
        ...
        SaleOrder = request.env['s2u.sale']
        ...
        order_count = SaleOrder.search_count([
            ('partner_id', '=', entity.id),
            ('state', 'in', ['payment', 'order', 'confirm', 'run', 'done'])
        ])
    ...
    def _order_check_access(self, order_id, access_token=None):
        order = request.env['s2u.sale'].browse([order_id])
    ...
    def portal_my_quotes(self, page=1, date_begin=None, date_end=None, sortby=None, **kw):
        ...
        searchbar_sortings = {
            'date': {'label': _('Order Date'), 'order': 'date_qu desc'},
        ...
        archive_groups = self._get_archive_groups('s2u.sale', domain)
        ...
        return request.render("s2usale.portal_my_quotations", values)
    ...
    def portal_my_orders(self, page=1, date_begin=None, date_end=None, sortby=None, **kw):
        ...
        domain = [
            ('partner_id', '=', entity.id),
            ('state', 'in', ['order', 'payment', 'confirm', 'run', 'done'])
        ]

        searchbar_sortings = {
            'date': {'label': _('Order Date'), 'order': 'date_so desc'},
            'name': {'label': _('Reference'), 'order': 'name'},
            'stage': {'label': _('Stage'), 'order': 'state'},
        }
        # default sortby order
        ...
        return request.render("s2usale.portal_my_orders", values)
    ...
    def portal_order_page(self, order=None, access_token=None, **kw):
        ...
        return request.render("s2usale.portal_order_page", values)
    ...
    def portal_order_report(self, order_id, access_token=None, **kw):
        ...
        if order_sudo.state in ['draft', 'quot', 'cancel']:
            pdf = request.env.ref('s2usale.s2usale_quotations_so').sudo().render_qweb_pdf([order_sudo.id])[0]
        else:
            pdf = request.env.ref('s2usale.s2usale_orders_so').sudo().render_qweb_pdf([order_sudo.id])[0]
        pdfhttpheaders = [
    ...
    def _portal_quote_user_can_accept(self, order_id):
        return request.env['ir.config_parameter'].sudo().get_param('s2usale.sale_portal_confirmation_options', default='none') in ('pay', 'sign')
