odoo.define('s2uproject.update_kanban', function (require) {
'use strict';
...
KanbanRecord.include({
    ...
    _openRecord: function () {
        if (this.modelName === 's2u.project' && this.$(".o_project_kanban_boxes a").length) {
    ...
    _onKanbanActionClicked: function (ev) {
        var self = this;
        if (this.modelName === 's2u.project.task' && $(ev.currentTarget).data('type') === 'set_cover') {
            ev.preventDefault();

            var domain = [['res_model', '=', 's2u.project.task'], ['res_id', '=', this.id], ['mimetype', 'ilike', 'image']];
            this._rpc({
                    model: 'ir.attachment',
                    method: 'search_read',
                    domain: domain,
        ...
        function open_cover_images_dialog(attachment_ids) {
            ...
            var $content = $(QWeb.render("s2uproject.SetCoverModal", {
                cover_id: coverID,
